...
export class IsraelidatePipe implements PipeTransform {
  ...
  transform(value: Date, ...args: unknown[]): string {
    ...
    let hebrewMonths:string[] = ['ינואר', 'פברואר','מרץ','אפריל' ,'מאי'  ,'יוני','יולי','אוגוסט','ספטמבר','אוקטובר','נובמבר','דצמבר']
    let hebrewDate:string = day + ' ב'+hebrewMonths[Number(month)-1] + ', ' + year;
    return hebrewDate;
  }
